...
function startGame() {
  $("yes").disabled = false;
  $("no").disabled  = false;
  getNode(1);   // initialize the game at the first node of data
}

// Fetches the next node of the game using Ajax.
function getNode(id) {
  new Ajax.Request("/animalgame.php",
    {
      method: "get",
...
// Called when 'Yes' is clicked; requests next node and updates page.
function yesClick() {
  if (answerGiven) {
...
// Called when 'No' is clicked; requests next node and updates page.
function noClick() {
  if (answerGiven) {
